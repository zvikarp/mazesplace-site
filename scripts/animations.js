$(document).ready(function() {
  ...
  $("#maze-creator-board-size")
      .on("change", function(e) { updateBoardsSize(this.value.split("x")); });
  $("#maze-creator-brush-size")
      .on("change", function(e) { updateBrushSize(parseInt(this.value)); });

  function getXYFromEvent(event) {
    event.touches = event.touches || [];
    const x = event.offsetX ||
              event.touches[0].clientX + document.body.scrollLeft +
                  document.documentElement.scrollLeft - mazeCanvas.offsetLeft;
    const y = event.offsetY ||
              event.touches[0].clientY + document.body.scrollTop +
                  document.documentElement.scrollTop - mazeCanvas.offsetTop;
    return [ x, y ];
  }
  ...
  function validatePointXY(x, y) {
    const inRange = x < mazeCanvas.width - pnts.str.elmt.clientWidth &&
