$(document).ready(function() {
  ...
  $("#maze-creator-board-size")
      .on("change", function(e) { updateBoardSize(this.value); });
  ...
  function onMoveOnCanvas(e) {
    if (mouseDown) {
      e.touches = e.touches || [];
      const x = e.offsetX || e.touches[0].clientX + document.body.scrollLeft +
                                 document.documentElement.scrollLeft -
                                 canvas.offsetLeft;
      const y = e.offsetY || e.touches[0].clientY + document.body.scrollTop +
      ...
      const lastEventX =
          lastEvent.offsetX ||
          lastEvent.touches[0].clientX + document.body.scrollLeft +
              document.documentElement.scrollLeft - canvas.offsetLeft;
      const lastEventY =
          lastEvent.offsetY ||
          lastEvent.touches[0].clientY + document.body.scrollTop +
  ...
  function onMouseUpOnCanvas() { mouseDown = false; }
});
